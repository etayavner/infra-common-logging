...
const log4jsPropertiesFilePath = `${rootFolder}/etc/log4js-properties.json`;
...
function monitorLog4jsPropertiesFile() {
    ...
    fs.watch(log4jsPropertiesFilePath, async (event, filename) => {
        ...
        try {
            ...
            logger.info(`log4js-properties was changed and successfully reloaded!`)
        } catch (err) {
            logger.error(`log4js-properties was changed and could not be reloaded: ${err.message}`)
        } finally {
            fsWait = false;
        }
    });
